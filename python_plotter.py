...
def plot_values_and_progression_3d(values, history):
    ...
    for idx, placements in enumerate(history):
        ...
        # Plot the placements as spheres on the surface
        ax.scatter(x_coords, y_coords, z_coords, color=colors[idx], s=200, depthshade=True, edgecolors="black", linewidth=1.5)
        ...
        if idx > 0:
            ...
            pz_coords = [values[int(y), int(x)] + 0.05 * np.max(values) for x, y in zip(px_coords, py_coords)]
            for px, py, pz, x, y, z in zip(px_coords, py_coords, pz_coords, x_coords, y_coords, z_coords):
                ax.plot([px, x], [py, y], [pz, z], color=colors[idx], linewidth=2.5, alpha=0.7)
    ...
    # Show the plot
    plt.tight_layout()
    plt.savefig("3d_placement_progression_plot_stylish.png", dpi=300)
    plt.show()
    ...
    lines = [ax.plot([], [], color=colors[i], linewidth=2.5, alpha=0.7)[0] for i in range(num_steps)]
    
    def update(frame):
        # Update scatter and line objects for the current frame
        for i in range(frame + 1):
            scatters[i].set_offsets(np.column_stack([history[i][1::2] + 0.5, history[i][::2] + 0.5]))
            if i > 0:
                lines[i-1].set_data([history[i-1][1::2] + 0.5, history[i][1::2] + 0.5], [history[i-1][::2] + 0.5, history[i][::2] + 0.5])
        return scatters + lines
    
    # Create the animation
    anim = FuncAnimation(fig, update, frames=num_steps, repeat=True, blit=True, interval=1000)
